-- Poule & Poulette Interne Audit Tool - Fix Enum with Separate Transactions
-- This script adds enum values in separate transactions to avoid commit issues
...
-- STAP 2: Add 'storemanager' enum value (separate transaction)
-- This must be run first and committed before any other operations
ALTER TYPE user_role ADD VALUE IF NOT EXISTS 'storemanager';

-- STAP 3: Add 'inspector' enum value (separate transaction)
ALTER TYPE user_role ADD VALUE IF NOT EXISTS 'inspector';

-- STAP 4: Add 'coo' enum value (separate transaction)
ALTER TYPE user_role ADD VALUE IF NOT EXISTS 'coo';

-- STAP 5: Add 'district_manager' enum value (separate transaction)
ALTER TYPE user_role ADD VALUE IF NOT EXISTS 'district_manager';

-- STAP 6: Add 'filiaal_manager' enum value (separate transaction)

-- Poule & Poulette Interne Audit Tool - Fix Enum Commit Issue
-- This script safely adds enum values and commits them before using them
...
-- STAP 2: Add enum values one by one and commit each one
-- This is crucial - each ALTER TYPE must be in a separate transaction
...
-- STAP 4: Now safely update existing users (only if rol column exists)
DO $$ 
BEGIN
...
SELECT 'Enum values committed and users updated successfully. Available roles: admin, inspector, storemanager' as status;

-- Poule & Poulette Interne Audit Tool - COMPLETE DATABASE FIX
-- Dit script herstelt alle database functies en zorgt ervoor dat alle app functionaliteit werkt
...
-- ========================================
-- STAP 5: Herstel alle database functies
-- ========================================
...
BEGIN
    ...
    WHERE (filiaal_id_param IS NULL OR a.filiaal_id = filiaal_id_param)
    AND (
        -- Admin en developer kunnen alles zien
        EXISTS (SELECT 1 FROM gebruikers WHERE id = user_id_param AND rol IN ('admin', 'developer'))
        OR
        -- Inspector kan acties zien van audits die zij hebben uitgevoerd
        (EXISTS (SELECT 1 FROM gebruikers WHERE id = user_id_param AND rol = 'inspector') AND a.district_manager_id = user_id_param)
        OR
        -- Store manager kan acties zien van hun filiaal
        (EXISTS (SELECT 1 FROM gebruikers WHERE id = user_id_param AND rol = 'storemanager') AND a.filiaal_id IN (
            SELECT id FROM filialen WHERE district_manager_id = user_id_param
        ))
        OR
        -- Gebruiker kan acties zien die aan hen zijn toegewezen
        ac.toegewezen_aan = user_id_param
    )
...
BEGIN
    ...
    AND (
        ...
        EXISTS (SELECT 1 FROM gebruikers WHERE id = user_id_param AND rol IN ('admin', 'developer'))
        OR
        -- Inspector kan audits zien die zij hebben uitgevoerd
        (EXISTS (SELECT 1 FROM gebruikers WHERE id = user_id_param AND rol = 'inspector') AND a.district_manager_id = user_id_param)
        OR
        -- Store manager kan audits zien van hun filiaal
        (EXISTS (SELECT 1 FROM gebruikers WHERE id = user_id_param AND rol = 'storemanager') AND a.filiaal_id IN (
            SELECT id FROM filialen WHERE district_manager_id = user_id_param
        ))
    )
...
BEGIN
    ...
    WHERE (
        ...
        EXISTS (SELECT 1 FROM gebruikers WHERE id = user_id_param AND rol IN ('admin', 'developer'))
        OR
        -- Store manager kan hun toegewezen filialen zien
        (EXISTS (SELECT 1 FROM gebruikers WHERE id = user_id_param AND rol = 'storemanager') AND f.district_manager_id = user_id_param)
        OR
        -- Inspector kan alle filialen zien
        EXISTS (SELECT 1 FROM gebruikers WHERE id = user_id_param AND rol = 'inspector')
    )
    ORDER BY f.naam;
END;
$$ LANGUAGE plpgsql;
...
-- ========================================
-- STAP 6: Update RLS policies voor juiste toegang
-- ========================================
...
CREATE POLICY "Filialen access based on role" ON filialen
    FOR SELECT USING (true); -- Iedereen kan filialen zien
...
-- ========================================
-- STAP 7: Voeg test data toe voor functionaliteit
-- ========================================
DO $$ BEGIN
    -- Voeg een test audit toe
    ...
    ON CONFLICT (id) DO UPDATE SET
        filiaal_id = EXCLUDED.filiaal_id, district_manager_id = EXCLUDED.district_manager_id, 
        audit_datum = EXCLUDED.audit_datum, status = EXCLUDED.status, 
        totale_score = EXCLUDED.totale_score, pass_percentage = EXCLUDED.pass_percentage, 
        opmerkingen = EXCLUDED.opmerkingen, updated_at = NOW();
    
    -- Voeg test audit resultaten toe
    INSERT INTO audit_resultaten (id, audit_id, checklist_item_id, resultaat, score, opmerkingen, verbeterpunt, created_at)
    VALUES 
        (
            'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb',
            'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa',
            (SELECT id FROM audit_checklist_items LIMIT 1),
            'ok',
            5,
            'Alles in orde',
            NULL,
        ...
        (
            ...
            'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa',
            (SELECT id FROM audit_checklist_items OFFSET 1 LIMIT 1),
            'niet_ok',
            2,
            'Verbetering nodig',
    ...
    ON CONFLICT (id) DO UPDATE SET
        resultaat = EXCLUDED.resultaat, score = EXCLUDED.score, 
        opmerkingen = EXCLUDED.opmerkingen, verbeterpunt = EXCLUDED.verbeterpunt;
    
    -- Voeg test acties toe
    INSERT INTO acties (id, audit_id, audit_resultaat_id, titel, beschrijving, urgentie, status, toegewezen_aan, deadline, created_at, updated_at)
    VALUES 
        (
            'dddddddd-dddd-dddd-dddd-dddddddddddd',
            'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa',
            'cccccccc-cccc-cccc-cccc-cccccccccccc',
            'Actie vereist: Schoonmaak verbeteren',
            'Schoonmaak verbeteren - Verbetering nodig',
            'high',
            'pending',
    ...
    ON CONFLICT (id) DO UPDATE SET
        ...
        urgentie = EXCLUDED.urgentie, status = EXCLUDED.status, 
        toegewezen_aan = EXCLUDED.toegewezen_aan, deadline = EXCLUDED.deadline, updated_at = NOW();
    
    RAISE NOTICE 'Test data created/updated';
END $$;
...
SELECT ac.id, ac.titel, ac.urgentie, ac.status, f.naam as filiaal
FROM acties ac
JOIN audits a ON ac.audit_id = a.id
JOIN filialen f ON a.filiaal_id = f.id
ORDER BY ac.created_at DESC;

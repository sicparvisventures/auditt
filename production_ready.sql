...
-- This enables RLS with proper policies for production deployment
-- Run this BEFORE deploying to Cloudflare Pages

-- First, ensure all data exists (from previous SQL)
...
DROP POLICY IF EXISTS "Allow all for authenticated users" ON filialen;
DROP POLICY IF EXISTS "Allow all for authenticated users" ON audit_checklist_items;
DROP POLICY IF EXISTS "Allow all for authenticated users" ON audits;
DROP POLICY IF EXISTS "Allow all for authenticated users" ON audit_resultaten;
DROP POLICY IF EXISTS "Allow all for authenticated users" ON acties;
DROP POLICY IF EXISTS "Allow all for authenticated users" ON rapporten;
DROP POLICY IF EXISTS "Allow all for authenticated users" ON notificaties;

-- PRODUCTION RLS POLICIES
-- For now, allow all authenticated users to access all data
-- This is a simplified approach that works with the current auth system

-- Users table - allow authenticated users to read all users
CREATE POLICY "Allow all for authenticated users" ON gebruikers
    FOR ALL TO authenticated
    USING (true)
    WITH CHECK (true);

-- Filialen table - allow authenticated users to access all filialen
CREATE POLICY "Allow all for authenticated users" ON filialen
    FOR ALL TO authenticated
    USING (true)
    WITH CHECK (true);

-- Checklist items - allow authenticated users to read
CREATE POLICY "Allow all for authenticated users" ON audit_checklist_items
    FOR ALL TO authenticated
    USING (true)
    WITH CHECK (true);

-- Audits - allow authenticated users to access all audits
CREATE POLICY "Allow all for authenticated users" ON audits
    FOR ALL TO authenticated
    USING (true)
    WITH CHECK (true);

-- Audit results - allow authenticated users to access all
CREATE POLICY "Allow all for authenticated users" ON audit_resultaten
    FOR ALL TO authenticated
    USING (true)
    WITH CHECK (true);

-- Actions - allow authenticated users to access all
CREATE POLICY "Allow all for authenticated users" ON acties
    FOR ALL TO authenticated
    USING (true)
    WITH CHECK (true);

-- Reports - allow authenticated users to access all
CREATE POLICY "Allow all for authenticated users" ON rapporten
    FOR ALL TO authenticated
    USING (true)
    WITH CHECK (true);

-- Notifications - allow authenticated users to access all
CREATE POLICY "Allow all for authenticated users" ON notificaties
    FOR ALL TO authenticated
    USING (true)
    WITH CHECK (true);
...
AND tablename IN ('gebruikers', 'filialen', 'audits', 'audit_resultaten', 'acties')
ORDER BY tablename;

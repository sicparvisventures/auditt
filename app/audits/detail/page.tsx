...
export default function AuditDetailPage() {
  ...
  useEffect(() => {
    if (!authLoading) {
      if (!user) {
        ...
        setError('Geen audit ID opgegeven')
        setLoading(false)
      }
    }
  }, [user, authLoading, router, auditId])
  ...
  const loadAudit = async () => {
    try {
      ...
      const auditData = await clientDataService.getAuditById(auditId)
      ...
      // Get audit results
      const results = await clientDataService.getAuditResults(auditId)
